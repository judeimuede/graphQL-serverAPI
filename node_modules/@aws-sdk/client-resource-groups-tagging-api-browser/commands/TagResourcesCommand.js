"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var __aws_sdk_middleware_stack = require("@aws-sdk/middleware-stack");
var TagResources_1 = require("../model/operations/TagResources");
var TagResourcesCommand = /** @class */ (function () {
    function TagResourcesCommand(input) {
        this.input = input;
        this.model = TagResources_1.TagResources;
        this.middlewareStack = new __aws_sdk_middleware_stack.MiddlewareStack();
    }
    TagResourcesCommand.prototype.resolveMiddleware = function (clientStack, configuration) {
        var handler = configuration.handler;
        var stack = clientStack.concat(this.middlewareStack);
        var handlerExecutionContext = {
            logger: {},
            model: this.model
        };
        return stack.resolve(handler(handlerExecutionContext), handlerExecutionContext);
    };
    return TagResourcesCommand;
}());
exports.TagResourcesCommand = TagResourcesCommand;
//# sourceMappingURL=TagResourcesCommand.js.map